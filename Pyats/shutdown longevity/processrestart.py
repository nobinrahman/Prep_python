...
def check_context(script_args):
    ...
    if output.count('Core location') and output.count('user requested') == 0 :
        ...
        module_args['sste_commands'] = '["show tech routing bgp"]'
        ...
        module_args = {}
        module_args['timeout'] = 7200
        module_args['sste_commands'] = '["show tech routing isis"]'
        stoutput = sste_common.exec_commands(module_args, script_args)
        if stoutput.count('harddisk:/showtech/'):
            stoutput = stoutput.split('harddisk:/showtech/')[-1].split()[0]
            showtech_dict = ['harddisk:/showtech/'+stoutput, stoutput]
        ...
        module_args = {}
        module_args['timeout'] = 7200
        module_args['sste_commands'] = '["show tech macsec"]'
        stoutput = sste_common.exec_commands(module_args, script_args)
        if stoutput.count('harddisk:/showtech/'):
            stoutput = stoutput.split('harddisk:/showtech/')[-1].split()[0]
            showtech_dict = ['harddisk:/showtech/' + stoutput, stoutput]
        ...
        module_args = {}
        module_args['timeout'] = 7200
        module_args['sste_commands'] = '["show tech healthcheck"]'
        stoutput = sste_common.exec_commands(module_args, script_args)
        if stoutput.count('harddisk:/showtech/'):
            stoutput = stoutput.split('harddisk:/showtech/')[-1].split()[0]
            showtech_dict = ['harddisk:/showtech/' + stoutput, stoutput]
...
@aetest.skip(reason='Skip ')
class Mem_Leak_Check_Start_BGP(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_start_bgp(self, steps, script_args, testscript, testbed, test_data, timing):
####################################### MEM_LEAK ####################################### 
...
@aetest.skip(reason='Skip ')
class processrestartBGP(aetest.Testcase):
    global coredump_list, showtech_list
    @aetest.setup
    def setup(self,steps,script_args,testbed,test_data,timing):
        ...
        self.ssh.execute('show platform')
        self.ssh.execute('show version')
        self.ssh.execute('clear context')
        self.ssh.execute('show context')
        self.ssh.execute('show memory summary location all')
        pass

    @aetest.test
    def restart(self,steps,script_args,testbed,test_data,timing):
        if script_args['os_type'] in ['5500', 'ncs5500', 'NCS5500']:
            if 'tgn' in test_data:
                ...
                log.info(ret_val)
            output = self.ssh.execute('show running-config router bgp')
            if output.count('router bgp') == 0:
                log.info('there is no bgp config in this box')
                return
            v4route = self.ssh.execute('show route summary | in bgp')
            if v4route.count('bgp'):
                v4route = v4route.split('bgp')[-1].split()[1]
            else:
                v4route = ''
            v6route = self.ssh.execute('show route ipv6 summary | in bgp')
            if v6route.count('bgp'):
                v6route = v6route.split('bgp')[-1].split()[1]
            else:
                v6route = ''
            for loop in range(int(self.loop)):
                log.info('##############Loop: ' + str(loop))
                self.ssh.execute('process restart bgp')
                time.sleep(5*60)
                v4routetemp = self.ssh.execute('show route summary | in bgp')
                if v4routetemp.count('bgp'):
                    v4routetemp = v4routetemp.split('bgp')[-1].split()[1]
                else:
                    v4routetemp = ''
                v6routetemp = self.ssh.execute('show route ipv6 summary | in bgp')
                if v6routetemp.count('bgp'):
                    v6routetemp = v6routetemp.split('bgp')[-1].split()[1]
                else:
                    v4routetemp = ''
                if v4routetemp != v4route :
                    log.warning('v4route does not matched')
                if v6routetemp != v6route :
                    log.warning('v6route does not matched')
...
@aetest.skip(reason='Skip ')
class Mem_Leak_Check_End_BGP(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_end_bgp(self, steps, script_args, testscript, testbed, test_data, timing):
...
#@aetest.skip(reason='Skip ')
class Mem_Leak_Check_Start_ISIS(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_start_ISIS(self, steps, script_args, testscript, testbed, test_data, timing):
...
#@aetest.skip(reason='Skip ')
class processrestartISIS(aetest.Testcase):
    global coredump_list, showtech_list
    @aetest.setup
    def setup(self,steps,script_args,testbed,test_data,timing):
    ...
    def restart(self,steps,script_args,testbed,test_data,timing):
        if script_args['os_type'] in ['5500', 'ncs5500', 'NCS5500']:
            if test_data['UUT'].count('F17'):
                log.info('no ISIS on F17, skip...')
                return
            if 'tgn' in test_data:
                log.info('Take_traffic_snapshot_before_trigger')
                ret_val = Take_traffic_snapshot_before_trigger(steps, script_args, testbed, test_data)
                log.info(ret_val)
            output = self.ssh.execute('show running-config router isis')
            if output.count('router isis') == 0:
                log.info('there is no isis config in this box')
                return
            v4route = self.ssh.execute('show route summary | in isis')
            if v4route.count('isis'):
                v4route = v4route.split('isis')[-1].split()[1]
            else:
                v4route = ''
            v6route = self.ssh.execute('show route ipv6 summary | in isis')
            if v6route.count('isis'):
                v6route = v6route.split('isis')[-1].split()[1]
            else:
                v6route = ''
            for loop in range(int(self.loop)):
                log.info('##############Loop: ' + str(loop))
                self.ssh.execute('process restart isis')
                time.sleep(5*60)
                v4routetemp = self.ssh.execute('show route summary | in isis')
                if v4routetemp.count('isis'):
                    v4routetemp = v4routetemp.split('isis')[-1].split()[1]
                else:
                    v4routetemp = ''
                v6routetemp = self.ssh.execute('show route ipv6 summary | in isis')
                if v6routetemp.count('isis'):
                    v6routetemp = v6routetemp.split('isis')[-1].split()[1]
                else:
                    v4routetemp = ''
                if v4routetemp != v4route :
                    log.warning('v4route does not matched')
...
#@aetest.skip(reason='Skip ')
class Mem_Leak_Check_End_ISIS(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_end_isis(self, steps, script_args, testscript, testbed, test_data, timing):
...
#@aetest.skip(reason='Skip ')
class Mem_Leak_Check_Start_MACSEC(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_start_MACSEC(self, steps, script_args, testscript, testbed, test_data, timing):
####################################### MEM_LEAK ####################################### 
...
#@aetest.skip(reason='Skip ')
class processrestartMACSEC(aetest.Testcase):
    ...
    @aetest.test
    def restart_ea(self,steps,script_args,testbed,test_data,timing):
        if 'tgn' in test_data:
            log.info('Take_traffic_snapshot_before_trigger')
            ret_val = Take_traffic_snapshot_before_trigger(steps, script_args, testbed, test_data)
            log.info(ret_val)
        summarybeforrestart = self.ssh.execute('show macsec mka summary')
        summarybeforrestart = 'Total' + summarybeforrestart.split('Total')[-1]
        log.info(summarybeforrestart)
        for loop in range(int(self.loop)):
            log.info('##############Loop: ' + str(loop))
            self.ssh.execute('process restart macsec_ea')
            time.sleep(5*60)
            summaryafterrestart = self.ssh.execute('show macsec mka summary')
            summaryafterrestart = 'Total' + summaryafterrestart.split('Total')[-1]
            log.info(summaryafterrestart)
            if summaryafterrestart != summarybeforrestart :
                show_tech(script_args)
                self.failed('macsec session summary does not matched\n')
            if check_context(script_args):
                self.failed('crash happened\n')
            self.ssh.execute('show memory summary location all')
            if 'tgn' in test_data:
    ...
    @aetest.test
    def restart_mka(self,steps,script_args,testbed,test_data,timing):
        if 'tgn' in test_data:
            log.info('Take_traffic_snapshot_before_trigger')
            ret_val = Take_traffic_snapshot_before_trigger(steps, script_args, testbed, test_data)
            log.info(ret_val)
        ...
        for loop in range(int(self.loop)):
            log.info('##############Loop: ' + str(loop))
            self.ssh.execute('process restart macsec_mka')
            time.sleep(5*60)
            summaryafterrestart = self.ssh.execute('show macsec mka summary')
            summaryafterrestart = 'Total' + summaryafterrestart.split('Total')[-1]
            log.info(summaryafterrestart)
...
#@aetest.skip(reason='Skip ')
class Mem_Leak_Check_End_MACSEC(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_end_MACSEC(self, steps, script_args, testscript, testbed, test_data, timing):
...
@aetest.skip(reason='Skip ')
class Mem_Leak_Check_Start_HCmgr(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_start_HCmgr(self, steps, script_args, testscript, testbed, test_data, timing):
####################################### MEM_LEAK ####################################### 
...
@aetest.skip(reason='Skip ')
class processrestartHCmgr(aetest.Testcase):
    global coredump_list, showtech_list
    @aetest.setup
    def setup(self,steps,script_args,testbed,test_data,timing):
        ...
        self.ssh.execute('show context')
        self.ssh.execute('show memory summary location all')
        output = self.ssh.execute('show install active summary')
        if output.count('health') == 0:
            self.failed('no healthcheck rpm on this box\n')
        pass

    @aetest.test
    def restart_hcmgr(self,steps,script_args,testbed,test_data,timing):
        if 'tgn' in test_data:
            log.info('Take_traffic_snapshot_before_trigger')
            ret_val = Take_traffic_snapshot_before_trigger(steps, script_args, testbed, test_data)
            log.info(ret_val)
        metrics = ["show healthcheck metric asic-errors  summary | in available",
                   "show healthcheck metric fabric-stats  summary | in available",
                   "show healthcheck metric cpu | in available",
                   "show healthcheck metric fabric-health  | in available",
                   "show healthcheck metric filesystem | in available",
                   "show healthcheck metric fpd | in available",
                   "show healthcheck metric free-mem | in available",
                   "show healthcheck metric platform | in available"
                   "show healthcheck metric interface-counters summary | in available",
                   "show healthcheck metric redundancy | in available",
                   "show healthcheck metric shared-mem | in available"]
        output = self.ssh.execute('show healthcheck status | in Health')
        if output.count('Enabled') == 0:
            self.ssh.execute('show tech-support healthcheck')
            self.ssh.execute('show logging')
            show_tech(script_args)
            self.failed('healthcheck is not enable\n')
        for metric in metrics:
            output = self.ssh.execute(metric)
            if output.count('No data available for display'):
                show_tech(script_args)
                self.failed(metric + 'not work\n')
        self.ssh.execute('show healthcheck report')
        output = self.ssh.execute('show version')
        if output.count('7.3.4') or output.count('7.5.4'):
            self.ssh.execute('show healthcheck use-case packet-drop summary')
            self.ssh.execute('show healthcheck use-case asic-reset summary')
            self.ssh.execute('show healthcheck use-case packet-drop detail npu all location all')
            self.ssh.execute('show healthcheck use-case asic-reset detail npu all location all')
        
        for loop in range(int(self.loop)):
            log.info('##############Loop: ' + str(loop))
            self.ssh.execute('process restart hcmgr')
            time.sleep(5*60)
...
@aetest.skip(reason='Skip ')
class Mem_Leak_Check_End_HCmgr(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_end_HCmgr(self, steps, script_args, testscript, testbed, test_data, timing):
...
#@aetest.skip(reason='Skip ')
class Mem_Leak_Check_Start_invmgr(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_start_invgr(self, steps, script_args, testscript, testbed, test_data, timing):
####################################### MEM_LEAK ####################################### 
...
#@aetest.skip(reason='Skip ')
class processrestartsdr_invmgr(aetest.Testcase):
    ...
    @aetest.test
    def restart_sdr_invmgr(self,steps,script_args,testbed,test_data,timing):
        if 'tgn' in test_data:
            log.info('Take_traffic_snapshot_before_trigger')
            ret_val = Take_traffic_snapshot_before_trigger(steps, script_args, testbed, test_data)
            log.info(ret_val)
        if script_args['os_type'] in ['5500','ncs5500','NCS5500']:
            summarybeforrestart = self.ssh.execute('show sdr')
            summarybeforrestart = summarybeforrestart.split('PartnerName')[-1]
            log.info(summarybeforrestart)
        for loop in range(int(self.loop)):
            log.info('##############Loop: ' + str(loop))
            self.ssh.execute('process restart sdr_invmgr')
            time.sleep(5*60)
            if script_args['os_type'] in ['5500','ncs5500','NCS5500']:
                summaryafterrestart = self.ssh.execute('show sdr')
                summaryafterrestart = summaryafterrestart.split('PartnerName')[-1]
                log.info(summaryafterrestart)
                if summaryafterrestart != summarybeforrestart :
                    show_tech(script_args)
                    self.failed('show sdr summary does not matched\n')
            if check_context(script_args):
                self.failed('crash happened\n')
            self.ssh.execute('show memory summary location all')
            if 'tgn' in test_data:
...
#@aetest.skip(reason='Skip ')
class Mem_Leak_Check_End_invmgr(aetest.Testcase):
    #global coredump_list, showtech_list, interface_list

    @aetest.test
    def memleak_check_end_invmgr(self, steps, script_args, testscript, testbed, test_data, timing):
