...
def show_tech(script_args):
    ...
    module_args = {}
    module_args['timeout'] = 7200
    module_args['sste_commands'] = '["show tech-support fabric"]'
    stoutput = sste_common.exec_commands(module_args, script_args)
    if stoutput.count('harddisk:/showtech/'):
        stoutput = stoutput.split('harddisk:/showtech/')[-1].split()[0]
        showtech_dict = ['harddisk:/showtech/'+stoutput, stoutput]
            ...
            if True:
                ...
                while notseen:
                    ...
                    if tgn_retry_count == 0:
                        log.info(banner( "One or more traffic streams not converge"))
                        log.info(str(notseen))
                        converged = False
                        break
...
class CommonSetup(aetest.CommonSetup):
    ...
    def setup(self,test_data):
        aetest.loop.mark(reload_fc, testcase=['reload_fc'])
...
class reload_fc(aetest.Testcase):
    global coredump_list, showtech_list
    @aetest.test
    def reload_fc(self,steps,script_args,testbed,test_data,timing):
        ...
        try:
            module_args = {}
            module_args['sste_commands'] = '["show platform | in FC | in OPERATIONAL"]'
            if script_args['os_type'] in ['8000']:
                module_args['sste_commands'] = '["show platform | in FC | in OPERATIONAL"]'
            output = sste_common.exec_commands(module_args, script_args)
            ...
            for index in range(1, len(output)):
                tmp = output[index]
                FCs.append(tmp.split()[0])
            log.info('OPERATIONAL FC:')
            log.info(FCs)
        except Exception as e:
            log.error(str(e))
            show_tech(script_args)
            self.failed("show platform | in FC | in OPERATIONAL: Failed")
        ...
        for index in range(int(self.loop)):
            ...
            if 'tgn' in test_data:
                ...
                log.info(ret_val)
            self.fc = str(random.choice(FCs))

            module_args = {}
            module_args['sste_commands'] = '["show platform | in 0/' + self.fc +'"]'
            output = sste_common.exec_commands(module_args, script_args)
            if output.count('OPERATIONAL') == 0:
                show_tech(script_args)
                self.failed('\nFC is not in OPERATIONAL before reload')

            ##check fabric plane
            fabric_plane_before_reload = ''
            module_args = {}
            if script_args['os_type'] in ['5500', 'ncs5500', 'NCS5500']:
                module_args['sste_commands'] = '["admin show controller fabric plane all"]'
            if script_args['os_type'] in ['8000']:
                module_args['sste_commands'] = '["show controller fabric plane all"]'
            fabric_plane_before_reload = sste_common.exec_commands(module_args, script_args)
            fabric_plane_before_reload = 'Plane Admin' + fabric_plane_before_reload.split('Plane Admin')[-1]


            module_args = {}
            if script_args['os_type'] in ['5500','ncs5500','NCS5500']:
                module_args['sste_commands'] = '["admin hw-module location 0/' + self.fc + ' reload noprompt"]'
            if script_args['os_type'] in ['8000']:
                module_args['sste_commands'] = '["reload location 0/' + self.fc + ' noprompt"]'
            ...
            if output.count('OPERATIONAL') == 0:
                show_tech(script_args)
                self.failed('\nFC is not in OPERATIONAL after reload')

            ##check fabric plane
            fabric_plane_after_reload = ''
            module_args = {}
            if script_args['os_type'] in ['5500', 'ncs5500', 'NCS5500']:
                module_args['sste_commands'] = '["admin show controller fabric plane all"]'
            if script_args['os_type'] in ['8000']:
                module_args['sste_commands'] = '["show controller fabric plane all"]'
            fabric_plane_after_reload = sste_common.exec_commands(module_args, script_args)
            fabric_plane_after_reload = 'Plane Admin' + fabric_plane_after_reload.split('Plane Admin')[-1]

            log.info('fabric_plane_before_reload:\n'+fabric_plane_before_reload)
            log.info('fabric_plane_after_reload:\n' + fabric_plane_after_reload)

            fabric_plane_before_reload = fabric_plane_before_reload.count('UP')
            fabric_plane_after_reload = fabric_plane_after_reload.count('UP')

            if fabric_plane_after_reload == fabric_plane_before_reload:
                pass
            else:
                show_tech(script_args)
                self.failed('\nshow controller fabric plane all changed before/after reload')
            ...
            # ###debug
            # module_args = {}
            # module_args['sste_commands'] = '["dumpcore running hcmgr"]'
            # output = sste_common.exec_commands(module_args, script_args)
