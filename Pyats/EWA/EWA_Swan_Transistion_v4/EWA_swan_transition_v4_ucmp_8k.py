...
def V6_Controller(failed, steps, script_args, testscript, testbed, test_data, timing):
    ...
    controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D18-4NH-500CBF-V4+V6-V4-UCMP.xml | curl -s -X POST http://172.25.124.54:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
    time.sleep(2)
    ...
    time.sleep(2)
    controller.execute('gzip -c /root/MBB_MASTER/MIXED/UCMP/OWR1/OWR1_D8WAN_SLICE_500X16_PB_UCMP_V4V6NH_8K.XML | curl -s -X POST http://172.25.124.52:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
    time.sleep(2)
    controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D4WAN-4NH-500CBF-V4+V6-V4-UCMP.xml | curl -s -X POST http://172.25.124.79:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
    time.sleep(10)
    controller.disconnect()
...
class TC1_CONTINUOUS_SWAN_PROGRAMMING(aetest.Testcase):
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for beloop in range(int(self.beloop)):
            ...
            time.sleep(1)
            self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D4WAN-4NH-500CBF-V4+V6-V4-UCMP.xml | curl -s -X POST http://172.25.124.79:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
            time.sleep(10)
            ...
            for beloop in range(int(self.beloop)):
                ...
                if output.count('HW_PROG_ERROR'):
                    self.D4WAN.configure('no interface FH0/0/0/0 shutdown\n'
                                    ...
                                    'no interface Hu0/3/0/24 shutdown\n', timeout=300)
                    self.failed('Hardware Programming Error Found\n')
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                self.controller.execute('gzip -c /root/MBB_MASTER/MIXED/UCMP/OWR1/OWR1_D8WAN_SLICE_500X16_PB_UCMP_V4V6NH_8K.XML | curl -s -X POST http://172.25.124.52:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
                time.sleep(10)
                self.controller.disconnect()
