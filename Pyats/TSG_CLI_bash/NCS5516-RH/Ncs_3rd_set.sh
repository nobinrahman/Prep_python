...
while {$infinite_loop == 1} {
    ...
    expect "#"
    send "show controllers npu stats asic-counters instance all location all | in \"Statistics|dropped_packets\" | in \"Statistics|pdvoq_shared_mma\"\r"
    sleep 10

    expect "#"
    send "show controllers npu driver location 0/RP0/CPU0\r"
    sleep 10
