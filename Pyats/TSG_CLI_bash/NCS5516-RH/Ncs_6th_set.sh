...
while {$infinite_loop == 1} {
    ...
    expect "#"
    send "show controllers npu debugshell 0 \"script ip_chain 0 0.0.0.0\" location all\r"
    sleep 10

    expect "#"
    send "show controllers npu stats traps-all instance all location all | utility egrep 'for lc|Dropped|L3_ROUTE'\r"
    sleep 10
