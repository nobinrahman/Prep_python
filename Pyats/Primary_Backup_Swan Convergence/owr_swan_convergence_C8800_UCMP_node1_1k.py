...
class SWAN_Convergence_UCMPv4_4k_Node1(aetest.Testcase):
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            #self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D18-4NH-500V4-500V6-to-250-V6-V4-UCMP.xml | curl -s -X POST http://172.25.124.54:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
            #self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D4-4NH-500V4-500V6-to-250-V6-V4-UCMP.xml | curl -s -X POST http://172.25.124.79:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            ...
            DBF26 = float(flowdefault_sort[0]) / 1000000            
            DWF26 = float(flowdefault_sort[-1]) / 1000000

            
            log.info ('FLOWOUTPUT_SCAVENGER_BEST: %s' %flowscavenger_sort[0])
            log.info ('FLOWOUTPUT_SCAVENGER_WORST: %s' %flowscavenger_sort[-1])

            SBF26 = float(flowscavenger_sort[0]) / 1000000
            SWF26 = float(flowscavenger_sort[-1]) / 1000000

            log.info('####T19:' + T19)
            Aggrigate26 = float(T19) / 1000000
