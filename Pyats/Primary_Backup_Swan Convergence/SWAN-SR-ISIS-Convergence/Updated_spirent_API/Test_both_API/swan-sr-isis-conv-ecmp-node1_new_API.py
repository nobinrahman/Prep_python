...
def Take_traffic_snapshot(steps, script_args, testbed, test_data):
    if 'tgn' in test_data:
        with steps.start("Take Traffic Snapshot", continue_=True) as s:
            try:
                errors = []
                # log.info("Clear Traffic Stats")
                # sste_tgn.tgn_clear_stats(script_args, test_data)
                ...
                for stream, item in script_args['tgn_snapshot'].items():
                    ...
                    # Check if loss_percent is not 0.0
                    for stream_name, stream_data in tgn_stat.items():
                        if 'loss_percent' in stream_data and stream_data['loss_percent'] != 0.0:
                            log.info("This is not expected. Loss Percent is not 0.0.")
                            break
            except Exception as e:
                log.error(str(e))
                s.failed(step_txt + ": Failed")
...
class Convergence_Test_ECMPv4_4k_Node1(aetest.Testcase):
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            data_swan.append(["Ingress_Primary_4NH","Shut"] + get_values_non_zero(test_data))


        # Taking traffic snapshot after trigger
        if 'tgn' in test_data:
            log.info('Take_traffic_snapshot_before_trigger')
            ret_val = Take_traffic_snapshot(steps, script_args, testbed, test_data)
            log.info(ret_val)
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            data_swan.append(["Ingress_Primary_4NH","No Shut"] + get_values_non_zero(test_data))
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            data_swan.append(["Ingress_Default_ALL_Scavenger_Primary_6NH","No Shut"] + get_values(test_data))
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            data_swan.append(["Ingress_Scavenger_ALL_Default_Primary_6NH","No Shut"] + get_values(test_data))
        ...
        if 'tgn' in test_data:
            ...
            log.info(banner("Printing data with New API"))
            ...
            file_path_swan = "/scratch/msftcvtauto/WAN/SWAN/CONVERGENCE/Logs/ecmp_node1_swan_new.csv"
