...
def V4_Controller(failed, steps, script_args, testscript, testbed, test_data, timing):
    ...
    time.sleep(2)
    controller.execute('cd /root/MBB_MASTER/MIXED && ./ykn_mixed-clis-ecmp.sh', timeout = 300)
...
class TC1_CONTINUOUS_SWAN_PROGRAMMING(aetest.Testcase):
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for beloop in range(int(self.beloop)):
            ...
            time.sleep(1)
            self.controller.execute('cd /root/MBB_MASTER/MIXED && ./ykn_mixed-clis-ecmp.sh', timeout = 300)
        ...
        self.EWA3 = genietestbed.devices['EWA3']
        retry = 0
        while retry < 10:
            try:
                self.EWA3.connect(via='vty', connection_timeout=600, mit=True)
                break
            except:
                time.sleep(300)
                retry += 1
                if retry == 10:
                    log.failed('connect failed')
        log.info(banner("Checking Hardware Programming Error"))
        output = self.EWA3.execute('show logging | in HW_PROG_ERROR', timeout = 300)
        if output.count('HW_PROG_ERROR'):
            self.failed('Hardware Programming Error Found\n')
        time.sleep(10)
        log.info(banner("Checking Out of Resource Error"))
        output = self.EWA3.execute('show logging | in OOR_RED | exclude sipidxtbl', timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.EWA3.execute('clear log', timeout = 300)
                self.EWA3.execute('clear context', timeout = 300)
                self.EWA3.configure('interface FH0/0/0/14 shutdown\n'
                                    'interface FH0/0/0/15 shutdown\n'
                                    'interface FH0/0/0/16 shutdown\n'
                                    'interface FH0/0/0/17 shutdown\n', timeout=300)
                ...
                if output.count('HW_PROG_ERROR'):
                    self.EWA3.configure('no interface FH0/0/0/14 shutdown\n'
                                    'no interface FH0/0/0/15 shutdown\n'
                                    'no interface FH0/0/0/16 shutdown\n'
                                    'no interface FH0/0/0/17 shutdown\n', timeout=300)
                    self.failed('Hardware Programming Error Found\n')
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D8WAN.configure('interface Bundle-Ether42500\n'
                ...
                self.EWA3.execute('show arp | i 42500', timeout = 300)

                output = self.EWA3.execute('ping 213.6.0.1', timeout = 300)
                if output.count('!'):
                    log.info('Ping test working fine')
                else: 
                    self.failed('Ping test not working')


                output = self.EWA3.execute('show cef ipv4 100.100.110.1/32 detail | i "Bundle-Ether|labels imposed" | e Y', timeout=300)
                ...
                output = self.EWA3.execute('show cef ipv6 3000::1 detail | i "Bundle-Ether|labels imposed" | e Y', timeout=300)
                # Define the regular expression pattern to capture "labels imposed"
                ...
                output = self.EWA3.execute('show mpls forwarding prefix 100.100.110.1/32', timeout=300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.EWA3.execute('clear log', timeout = 300)
                self.EWA3.execute('clear context', timeout = 300)
                self.EWA3.configure('interface FH0/0/0/14\n'
                                    'no bundle id 42500 mode active\n'
                                    'interface FH0/0/0/15\n'
                                    'no bundle id 42501 mode active\n'
                                    'interface FH0/0/0/16\n'
                                    'no bundle id 42502 mode active\n'
                                    'interface FH0/0/0/17\n'
                                    'no bundle id 42503 mode active\n', timeout=60)
                ...
                self.EWA3.configure('interface FH0/0/0/14\n'
                                    'bundle id 42500 mode active\n'
                                    'interface FH0/0/0/15\n'
                                    'bundle id 42501 mode active\n'
                                    'interface FH0/0/0/16\n'
                                    'bundle id 42502 mode active\n'
                                    'interface FH0/0/0/17\n'
                                    'bundle id 42503 mode active\n', timeout=60)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.EWA3.execute('reload location 0/3/CPU0 noprompt', timeout = 7200)
                except:
                ...
                while True:
                    output = self.EWA3.execute('sh platform | i 0/3/CPU0',timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.EWA3.configure('hw-module shutdown location 0/0/CPU0', timeout = 7200)
                except:
                ...
                try:
                    self.EWA3.configure('no hw-module shutdown location 0/0/CPU0', timeout = 7200)
                except:
                ...
                while True:
                    output = self.EWA3.execute('sh platform | i 0/0/CPU0',timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D8WAN.configure('interface FH0/3/0/31\n'
                                    'no bundle id 42500 mode active\n'
                                    'interface FH0/3/0/32\n'
                                    'no bundle id 42501 mode active\n'
                                    'interface FH0/3/0/33\n'
                                    'no bundle id 42502 mode active\n'
                                    'interface FH0/3/0/34\n'
                                    'no bundle id 42503 mode active\n', timeout=300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.D8WAN.execute('reload location 0/3/CPU0 noprompt', timeout = 7200)
                except:
                    log.info("Lincard still reloading")
                    time.sleep(300)
                ...
                while True:
                    output = self.D8WAN.execute('sh platform | i 0/3/CPU0',timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.D8WAN.configure('hw-module shutdown location 0/3/CPU0', timeout = 7200)
                except:
                    log.info("Lincard is shutting down")
                    time.sleep(300)
                ...
                try:
                    self.D8WAN.configure('no hw-module shutdown location 0/3/CPU0', timeout = 7200)
                except:
                    log.info("Lincard is booting")
                    time.sleep(300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                output = self.controller.execute('gzip -c /root/MBB_MASTER/Incorrect_D4WAN_hostname.xml | curl -s -X POST http://172.25.124.40:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                output = self.controller.execute('curl -v http://172.25.124.40:10000/version', timeout = 300)
                ...
                output = self.controller.execute('curl -v http://172.25.124.40:10000/flowtable', timeout = 300)
                ...
                output = self.controller.execute('curl -v http://172.25.124.40:10000/flowtable/summary', timeout = 300)
                ...
                output = self.controller.execute('curl -v http://172.25.124.40:10000/tunnels | wc -l', timeout = 300)
                ...
                output = self.controller.execute('curl -v http://172.25.124.40:10000/instance/1/version', timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.EWA3.execute('show run linux networking', timeout = 300)
                self.EWA3.execute('sh run linux networking | file harddisk:/linux_networking_permit.cfg', timeout = 300)
                self.EWA3.execute('sh run linux networking | e permit | file harddisk:linux_networking_deny.cfg', timeout = 300)


                log.info(banner("Denying Linux Networking"))

                self.EWA3.configure('no linux networking', timeout=600)

                self.EWA3.execute('show run linux networking', timeout = 300)

                self.EWA3.configure('load harddisk:/linux_networking_deny.cfg', timeout=600)


                self.EWA3.execute('show run linux networking | e permit', timeout = 300)
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/EWA3-4NH-1000CBF-6K-Impose-24x-ECMP.xml | curl -s -X POST http://172.25.124.74:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
                self.controller.disconnect()
                time.sleep(120)
                output = self.EWA3.execute('show cef ipv4 100.100.110.1/32 detail | i "Bundle-Ether|labels imposed" | e Y', timeout=300)
                # Define the regular expression pattern to capture "labels imposed"
                ...
                self.EWA3.configure('load harddisk:/linux_networking_permit.cfg', timeout=600)
...
#                            log.failed('connect failed')
                ...
                output = self.EWA3.execute('show cef ipv4 100.250.104.99/32 detail loca 0/0/cpu0', timeout = 300)
                #
                #
                #
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.EWA3.execute('bash ifconfig | awk \'/^vethac100005/,/^$/\'', timeout=300)
                self.EWA3.execute('bash ifconfig | awk \'/^vethac100001/,/^$/\'', timeout=300)
                self.EWA3.execute('show ipv4 int brief | i Mgmt', timeout=300)
                self.EWA3.execute('show run | i virtu', timeout=300)

                output = self.EWA3.execute('bash docker ps -a', timeout=300)
                ...
                output = self.EWA3.execute('bash docker logs ' + str(container_id1), timeout = 300)
                ...
                output = self.EWA3.execute('bash docker logs ' + str(container_id2), timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.EWA3.execute('show appmgr application-table', timeout = 300)
                self.EWA3.execute('show appmgr source-table', timeout = 300)
                self.EWA3.execute('show appmgr packages installed', timeout = 300)
                self.EWA3.execute('show appmgr process-script-table', timeout = 300)
                self.EWA3.execute('show appmgr source name swanagent', timeout = 300)
                self.EWA3.execute('show appmgr application name SwanAgent info detail', timeout = 300)
                self.EWA3.execute('show appmgr application name SwanAgent info summary', timeout = 300)
                self.EWA3.execute('show appmgr application name SwanAgent logs', timeout = 300)
                self.EWA3.execute('show appmgr application name SwanAgent logs ', timeout = 300)
                self.EWA3.execute('show appmgr application name SwanAgent stats', timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            for beloop in range(int(self.beloop)):
                ...
                output = self.EWA3.execute('show appmgr application name SwanAgent logs | i Time in ms', timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.EWA3.execute('sh appmgr application-table', timeout = 300)
                output= self.EWA3.execute('sh process cpu | i agentxr', timeout = 300)
                ...
                self.EWA3.execute('bash kill -9 ' + str(swanagent_process), timeout = 300)

                time.sleep(30)

                output = self.EWA3.execute('sh appmgr application-table', timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.EWA3.execute('bash kill -HUP ' + str(swanagent_process), timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            for beloop in range(int(self.beloop)):
                ...
                output = self.EWA3.execute('run pidof agentxr', timeout = 300)
                output = output.split()

                PID1_before = output[-1]
                print(f"PID of Swanagent before: {PID1_before}")

                output = self.EWA3.execute('run pidof vrf_relay', timeout = 300)


                output = output.split()

                PID2_before = output[-1]

                PID3_before = output[-2]

                print(f"PID of vrf-relay before: {PID2_before}" )
                print(f"PID of vrf-relay before: {PID3_before}" )


                output = self.EWA3.execute(f'run cd /proc/{PID1_before}/fd && ls -ltr | wc -l', timeout=300)
                lines = output.split()
                tun_4_before = lines[-1]
                print(tun_1_before)
        

                output = self.EWA3.execute(f'run cd /proc/{PID2_before}/fd && ls -ltr | wc -l', timeout=300)

                lines = output.split()
                tun_5_before = lines[-1]
                print(tun_2_before)
                

                output = self.EWA3.execute(f'run cd /proc/{PID3_before}/fd && ls -ltr | wc -l', timeout=300)

                lines = output.split()
                tun_6_before = lines[-1]
                print(tun_3_before)



                print(f"Captured PID1 before: {tun_1_before}")
                print(f"Captured PID2 before: {tun_2_before}")
                print(f"Captured PID3 before: {tun_3_before}")
                ...
                PID1_after = output[-1]
                print(f"PID of Swanagent before: {PID1_after}")
                ...
                print(f"PID of vrf-relay before: {PID2_after}" )
                print(f"PID of vrf-relay before: {PID3_after}" )


                output = self.EWA3.execute(f'run cd /proc/{PID1_after}/fd && ls -ltr | wc -l', timeout=300)
                lines = output.split()
                tun_1_after = lines[-1]
                print(tun_1_after)
        

                output = self.EWA3.execute(f'run cd /proc/{PID2_after}/fd && ls -ltr | wc -l', timeout=300)
                ...
                output = self.EWA3.execute(f'run cd /proc/{PID3_after}/fd && ls -ltr | wc -l', timeout=300)
                ...
                print(f"Captured PID2 before: {tun_2_before}")
                print(f"Captured PID3 before: {tun_3_before}")
                print(f"Captured PID1 before: {tun_1_after}")
                print(f"Captured PID2 before: {tun_2_after}")
                print(f"Captured PID3 before: {tun_3_after}")


                
                # Now, compare the numeric values
                if int(tun_1_after) <= int(tun_1_before) or int(tun_2_after) <= int(tun_2_before) or int(tun_3_after) <= int(tun_3_before):
