...
def V4_Controller(failed, steps, script_args, testscript, testbed, test_data, timing):
    ...
    time.sleep(2)
    controller.execute('cd /root/MBB_MASTER/MIXED && ./ewa_mixed-clis_ecmp.sh', timeout = 300)
    ...
    # controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D18-4NH-500CBF-V4+V6-V4-UCMP.xml | curl -s -X POST http://172.25.124.54:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
    # time.sleep(2)
    ...
    # time.sleep(2)
    # controller.execute('gzip -c /root/MBB_MASTER/MIXED/UCMP/OWR1/OWR1_D8WAN_SLICE_500X16_PB_UCMP_V4V6NH_8K.XML | curl -s -X POST http://172.25.124.52:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
    # time.sleep(60)
...
def Verifier_After(failed, steps, script_args, testscript, testbed, test_data, timing):
    ...
    while 1:
        ...
        if index == 120:
            log.error('###################\nlabels not recovered\n###################')
            break
    log.info('Waiting for 60 seconds for the traffic to flow')
    time.sleep(60)
    ########################Interface Accounting########################
    int_accounting = D8WAN.execute('show interfaces Bundle-Ether 28000 accounting rates', timeout = 300)
...
#@aetest.skip(reason='debug')
class TC1_CONTINUOUS_SWAN_PROGRAMMING(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        log.info(banner("TC1.0_Continuous_Swan_Programming")) 
        ...
        for beloop in range(int(self.beloop)):
            ...
            time.sleep(1)
            self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D12W_empty_fib.xml | curl -s -X POST http://172.25.124.75:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
            time.sleep(1)
            self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D12W_empty_fib.xml | curl -s -X POST http://172.25.124.75:10000/instance/1/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
            time.sleep(1)
            ...
            time.sleep(1)
            self.controller.execute('cd /root/MBB_MASTER/MIXED && ./ewa_mixed-clis_ecmp.sh', timeout = 300)
            ...
            # self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D18-4NH-500CBF-V4+V6-V4-UCMP.xml | curl -s -X POST http://172.25.124.54:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
            # time.sleep(1)
            ...
            # time.sleep(1)
            # self.controller.execute('gzip -c /root/MBB_MASTER/MIXED/UCMP/OWR1/OWR1_D8WAN_SLICE_500X16_PB_UCMP_V4V6NH_8K.XML | curl -s -X POST http://172.25.124.52:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
            # time.sleep(1)
            # self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D4WAN-4NH-1000CBF-6K-Impose-24x-UCMP.xml | curl -s -X POST http://172.25.124.79:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
            time.sleep(10)
        ...
        while 1:
            ...
            #     break
            # time.sleep(5)
            outputDefault = self.D8WAN.execute('show service-layer mpls label 24000 exp default | in priority', timeout=600)
            outputScavenger = self.D8WAN.execute('show service-layer mpls label 24000 exp 1 | in priority', timeout=600)
            if outputDefault.count('path up') == 3 and outputScavenger.count('path up') == 4:
                time.sleep(60)
                break
            time.sleep(5)
...
#@aetest.skip(reason='debug')
class TC2_LSP_UP_DOWN_BETWN_LOCAL_AND_REMOTE_PEER(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            log.info(banner("TC2.0-LSP up down between local nad remote peer"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                genietestbed = load(testbed_file)
                log.info(genietestbed.devices)
                self.D4WAN = genietestbed.devices['D4WAN']
                retry = 0
                while retry < 10:
                    try:
                        self.D4WAN.connect(via='vty', connection_timeout=600, mit=True)
                        break
                    except:
                        time.sleep(300)
                        retry += 1
                ...
                self.D4WAN.execute('clear log', timeout = 300)
                self.D4WAN.execute('clear context', timeout = 300)
                self.D4WAN.configure('interface FH0/0/0/0 shutdown\n'
                                    'interface FH0/3/0/20 shutdown\n'
                                    'interface FH0/0/0/1 shutdown\n'
                                    'interface FH0/3/0/21 shutdown\n'
                                    'interface FH0/0/0/2 shutdown\n'
                                    'interface FH0/0/0/3 shutdown\n'
                                    'interface FH0/0/0/9 shutdown\n', timeout=300)
                ...
                output = self.D4WAN.execute('show logging | in HW_PROG_ERROR', timeout = 300)

                if output.count('HW_PROG_ERROR'):
                    self.D4WAN.configure('no interface FH0/0/0/0 shutdown\n'
                                    'no interface FH0/3/0/20 shutdown\n'
                                    'no interface FH0/0/0/1 shutdown\n'
                                    'no interface FH0/3/0/21 shutdown\n'
                                    'no interface FH0/0/0/2 shutdown\n'
                                    'no interface FH0/0/0/3 shutdown\n'
                                    'no interface FH0/0/0/9 shutdown\n', timeout=300)
                    self.failed('Hardware Programming Error Found\n')

                log.info(banner("Checking Out of Resource Error"))
                output = self.D4WAN.execute('show logging | in OOR_RED | exclude sipidxtbl', timeout = 300)
...
#@aetest.skip(reason='debug')
class TC3_DMAC_CHANGE(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            log.info(banner("TC3.0-Dmac Change"))

            ################################## Verifier_Before ################################
            Verifier_Before(self.failed, steps, script_args, testscript, testbed, test_data, timing)

            ###################### D8WAN_Mac_Address_change ######################
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D8WAN.configure('interface Bundle-Ether8400\n'
                                   'mac-address 20.20.20', timeout=60)
                ...
                self.D4WAN.execute('show arp | i 8400', timeout = 300)

                output = self.D4WAN.execute('ping 104.108.0.1', timeout = 300)
                if output.count('!'):
                    log.info('Ping test working fine')


                output = self.D4WAN.execute('show cef ipv4 100.100.110.1/32 detail loca 0/0/cpu0 | i "Bundle-Ether|labels imposed" | e Y', timeout=300)
                ...
                output = self.D4WAN.execute('show cef ipv6 3000::1 detail loca 0/0/cpu0 | i "Bundle-Ether|labels imposed" | e Y', timeout=300)
                # Define the regular expression pattern to capture "labels imposed"
                ...
                output = self.D4WAN.execute('show mpls forwarding prefix 100.100.110.1/32', timeout=300)
                ...
                self.D8WAN.configure('interface Bundle-Ether8400\n'
                                   'no mac-address 20.20.20', timeout=60)
                self.D8WAN.disconnect()
                self.D4WAN.disconnect()
...
#@aetest.skip(reason='debug')
class TC4_INGRESS_LOCAL_BUNDLE_CONFIG_UNCONFIG(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            log.info(banner("TC4.0-Ingress Local Bundle Config Unconfig"))

            ################################## Verifier_Before ################################
            Verifier_Before(self.failed, steps, script_args, testscript, testbed, test_data, timing)

            ###################### D4WAN_BE_CONFIG_UNCONFIG######################
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D4WAN.execute('clear log', timeout = 300)
                self.D4WAN.execute('clear context', timeout = 300)
                self.D4WAN.configure('interface FH0/0/0/0\n'
                                    'no bundle id 8400 mode active\n'
                                    'interface FH0/3/0/20\n'
                                    'no bundle id 8400 mode active\n'
                                    'interface FH0/0/0/1\n'
                                    'no bundle id 8401 mode active\n'
                                    'interface FH0/3/0/21\n'
                                    'no bundle id 8401 mode active\n'
                                    'interface FH0/0/0/2\n'
                                    'no bundle id 8402 mode active\n'
                                    'interface FH0/0/0/3\n'
                                    'no bundle id 8403 mode active\n'
                                    'interface FH0/0/0/9\n'
                                    'no bundle id 8403 mode active', timeout=60)
                ...
                self.D4WAN.configure('interface FH0/0/0/0\n'
                                    'bundle id 8400 mode active\n'
                                    'interface FH0/3/0/20\n'
                                    'bundle id 8400 mode active\n'
                                    'interface FH0/0/0/1\n'
                                    'bundle id 8401 mode active\n'
                                    'interface FH0/3/0/21\n'
                                    'bundle id 8401 mode active\n'
                                    'interface FH0/0/0/2\n'
                                    'bundle id 8402 mode active\n'
                                    'interface FH0/0/0/3\n'
                                    'bundle id 8403 mode active\n'
                                    'interface FH0/0/0/9\n'
                                    'bundle id 8403 mode active', timeout=60)
...
#@aetest.skip(reason='debug')
class TC5_INGRESS_LOCAL_LC_RELOAD(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            log.info(banner("TC5.0-Ingress Local LC Reload"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.D4WAN.execute('reload location 0/3/CPU0 noprompt', timeout = 7200)
                except:
                ...
                while True:
                    output = self.D4WAN.execute('sh platform | i 0/3/CPU0',timeout = 300)
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                #self.controller.execute('gzip -c /root/MBB_MASTER/EXPLICIT_NULLV6_D4WAN-4NH-500V4-500V6-to-250-V6-V4-ECMP.xml | curl -s -X POST http://172.25.124.79:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
                self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D4WAN-4NH-1000CBF-6K-Impose-24x-UCMP.xml | curl -s -X POST http://172.25.124.79:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
            ...
            ##################################### Verifier_After ###################################
            Verifier_After(self.failed, steps, script_args, testscript, testbed, test_data, timing)
            log.info('Waiting 5 more minutes for all the traffic to converge')
            time.sleep(300)
...
#@aetest.skip(reason='debug')
class TC6_INGRESS_LOCAL_LC_SHUT_NOSHUT(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            log.info(banner("TC6.0-Ingress Local LC Shut NoShut"))

            ################################## Verifier_Before ################################
            Verifier_Before(self.failed, steps, script_args, testscript, testbed, test_data, timing)

            ###################### D4WAN_LC_Shut_Noshut######################
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.D4WAN.configure('hw-module shutdown location 0/0/CPU0', timeout = 7200)
                except:
                ...
                try:
                    self.D4WAN.configure('no hw-module shutdown location 0/0/CPU0', timeout = 7200)
                except:
                ...
                while True:
                    output = self.D4WAN.execute('sh platform | i 0/0/CPU0',timeout = 300)
...
#@aetest.skip(reason='debug')
class TC7_INGRESS_REMOTE_BUNDLE_CONFIG_UNCONFIG(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D8WAN.configure('interface FH0/1/0/0\n'
                                    'no bundle id 8400 mode active\n'
                                    'interface FH0/5/0/20\n'
                                    'no bundle id 8400 mode active\n'
                                    'interface FH0/1/0/1\n'
                                    'no bundle id 8401 mode active\n'
                                    'interface FH0/5/0/21\n'
                                    'no bundle id 8401 mode active\n'
                                    'interface FH0/1/0/2\n'
                                    'no bundle id 8402 mode active\n'
                                    'interface FH0/4/0/18\n'
                                    'no bundle id 8402 mode active\n'
                                    'interface FH0/1/0/3\n'
                                    'no bundle id 8403 mode active\n', timeout=300)
...
#@aetest.skip(reason='debug')
class TC8_INGRESS_REMOTE_LC_RELOAD(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            log.info(banner("TC8.0-Ingress Remote LC Reload"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.D8WAN.execute('reload location 0/1/CPU0 noprompt', timeout = 7200)
                except:
                    log.info("Lincard still reloading")
                    time.sleep(300)
                ...
                while True:
                    output = self.D8WAN.execute('sh platform | i 0/1/CPU0',timeout = 300)
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                self.controller.execute('gzip -c /root/MBB_MASTER/MIXED/ECMP/OWR1/OWR1_D8WAN_1000CBF_ECMP_nonshared_24x_HWPB.XML | curl -s -X POST http://172.25.124.52:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
                time.sleep(10)
                self.controller.disconnect()
...
#@aetest.skip(reason='debug')
class TC9_INGRESS_REMOTE_LC_SHUT_NOSHUT(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            log.info(banner("TC9.0-Ingress Remote LC Shut NoShut"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.D8WAN.configure('hw-module shutdown location 0/1/CPU0', timeout = 7200)
                except:
                    log.info("Lincard is shutting down")
                    time.sleep(300)
                ...
                try:
                    self.D8WAN.configure('no hw-module shutdown location 0/1/CPU0', timeout = 7200)
                except:
                    log.info("Lincard is booting")
                    time.sleep(300)
...
#@aetest.skip(reason='debug')
class TC10_INGRESS_REMOTE_ROUTER_RELOAD(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            log.info(banner("TC10.0-Ingress Remote Router Reload"))
...
#@aetest.skip(reason='debug')
class TC11_INVALID_NEXT_HOP_ADDRESS(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            log.info(banner("TC11.0-Invalid next hop address"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                output = self.controller.execute('gzip -c /root/MBB_MASTER/Incorrect_D4WAN_hostname.xml | curl -s -X POST http://172.25.124.54:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
...
#@aetest.skip(reason='debug')
class TC13_COMMUNICATION_WITH_SWAN_CONTROLLER(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            log.info(banner("TC13.0-Communication with Swan Controller"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                if output.count('24001'):
                    log.info('Total 24001 tunnels')
                else:
                    self.failed('24001 tunnels are not seen\n')
...
#@aetest.skip(reason='debug')
class TC14_DDoS_ATTACK_SURVIVAL_ON_PORT_10001_42000(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            log.info(banner("TC14.0-DDOS attack survival on port 10001 42000"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D4WAN.execute('show run linux networking', timeout = 300)
                self.D4WAN.execute('sh run linux networking | file harddisk:/linux_networking_permit.cfg', timeout = 300)
                self.D4WAN.execute('sh run linux networking | e permit | file harddisk:linux_networking_deny.cfg', timeout = 300)


                log.info(banner("Denying Linux Networking"))

                self.D4WAN.configure('no linux networking', timeout=600)

                self.D4WAN.execute('show run linux networking', timeout = 300)

                self.D4WAN.configure('load harddisk:/linux_networking_deny.cfg', timeout=600)


                self.D4WAN.execute('show run linux networking | e permit', timeout = 300)
                ...
                self.D4WAN.configure('load harddisk:/linux_networking_permit.cfg', timeout=600)
...
#@aetest.skip(reason='debug')
class TC18_LABEL_STAT_VERIFICATION(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            log.info(banner("TC18.0-Label Stat Verification"))
...
#@aetest.skip(reason='debug')
class TC19_MULTI_INSTANCE_SWAN_PROGRAMMING(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
...
#                            log.failed('connect failed')
                ...
                output = self.D4WAN.execute('show cef ipv4 100.250.104.99/32 detail loca 0/0/cpu0', timeout = 300)
                #
                #
                #
                # Need to put the code. check once xml is avaialble from Dhiren
                #
                #
                #
                log.info(banner("Checking Hardware Programming Error"))
...
#@aetest.skip(reason='debug')
class TC20_VRF_FWDING_ISSUE_CSCwb18777(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            log.info(banner("TC20.0-VRF fwding issue CSCwb18777"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D4WAN.execute('bash ifconfig | awk \'/^vethac100005/,/^$/\'', timeout=300)
                self.D4WAN.execute('bash ifconfig | awk \'/^vethac100001/,/^$/\'', timeout=300)
                self.D4WAN.execute('show ipv4 int brief | i Mgmt', timeout=300)
                self.D4WAN.execute('show run | i virtu', timeout=300)

                output = self.D4WAN.execute('bash docker ps -a', timeout=300)
                ...
                for line in lines:
                    parts = line.split()
                    if len(parts) >= 2 and parts[1] == "localhost/vrf-relay:latest":
                ...
                output = self.D4WAN.execute('bash docker logs ' + str(container_id1), timeout = 300)
                ...
                output = self.D4WAN.execute('bash docker logs ' + str(container_id2), timeout = 300)
...
#@aetest.skip(reason='debug')
class TC21_VERIFY_APPMGR_INFRA(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            log.info(banner("TC21.0-Verify Appmgr Infra"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D4WAN.execute('show appmgr application-table', timeout = 300)
                self.D4WAN.execute('show appmgr source-table', timeout = 300)
                self.D4WAN.execute('show appmgr packages installed', timeout = 300)
                self.D4WAN.execute('show appmgr process-script-table', timeout = 300)
                self.D4WAN.execute('show appmgr source name swanagent', timeout = 300)
                self.D4WAN.execute('show appmgr application name SwanAgent info detail', timeout = 300)
                self.D4WAN.execute('show appmgr application name SwanAgent info summary', timeout = 300)
                self.D4WAN.execute('show appmgr application name SwanAgent logs', timeout = 300)
                self.D4WAN.execute('show appmgr application name SwanAgent logs ', timeout = 300)
                self.D4WAN.execute('show appmgr application name SwanAgent stats', timeout = 300)
...
#@aetest.skip(reason='debug')
class TC22_LSP_PROGRAMMING_TIME(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            log.info(banner("TC22.0-LSP Programming Time"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                output = self.D4WAN.execute('show appmgr application name SwanAgent logs | i Time in ms', timeout = 300)
...
#@aetest.skip(reason='debug')
class TC23_SWAN_AGENT_CRASH(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            log.info(banner("TC23.0-SWAN Agent Crash"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D4WAN.execute('sh appmgr application-table', timeout = 300)
                output= self.D4WAN.execute('sh process cpu | i agentxr', timeout = 300)
                ...
                self.D4WAN.execute('bash kill -9 ' + str(swanagent_process), timeout = 300)

                time.sleep(30)

                output = self.D4WAN.execute('sh appmgr application-table', timeout = 300)
...
#@aetest.skip(reason='debug')
class TC24_SWAN_AGENT_RESTART(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            log.info(banner("TC24.0-SWAN Agent Restart"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D4WAN.execute('bash kill -HUP ' + str(swanagent_process), timeout = 300)
...
#@aetest.skip(reason='debug')
class TC25_RESOURCE_LEAK(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            log.info(banner("TC25.0-Resource Leak Test"))
            ...
            for beloop in range(int(self.beloop)):
                ...
                log.info(banner("Trigger: Resource Leak Test"))
                ...
                # Initialize three variables to store container IDs
                container_id_2 = None
                container_id_3 = None
                container_id_4 = None


                output= self.D4WAN.execute('run docker ps -a', timeout = 300)
                # Define the regular expression pattern to match the container IDs
                ...
                for idx, match in enumerate(matches[1:], start=2):
                    variable_name = f'container_id_{idx}'
                    print(f'{variable_name} = {match}')

                    # Save the container IDs in separate variables
                    if idx == 2:
                        container_id_2 = match
                    elif idx == 3:
                        container_id_3 = match
                    elif idx == 4:
                        container_id_4 = match

                process_1 = self.D4WAN.execute(f'run ps -ef | grep docker | grep {container_id_2}', timeout=300)
                lines = process_1.split('\n')
                # Look for the line containing "containerd-shim"
                pid_line = next(line for line in lines if 'containerd-shim' in line)
                # Extract the PID from the line
                pid_1 = pid_line.split()[1]
                print(f"Captured PID: {pid_1}")

                output = self.D4WAN.execute(f'run cd /proc/{pid_1}/fd && ls -ltr | wc -l', timeout=300)
                lines = output.split('\n')
                pid_1_before = lines[1].strip()
                


                process_2 = self.D4WAN.execute(f'run ps -ef | grep docker | grep {container_id_3}', timeout=300)
                lines = process_2.split('\n')
                # Look for the line containing "containerd-shim"
                pid_line = next(line for line in lines if 'containerd-shim' in line)
                # Extract the PID from the line
                pid_2 = pid_line.split()[1]
                print(f"Captured PID: {pid_2}")


                output = self.D4WAN.execute(f'run cd /proc/{pid_2}/fd && ls -ltr | wc -l', timeout=300)

                lines = output.split('\n')
                pid_2_before = lines[1].strip()
                

                process_3 = self.D4WAN.execute(f'run ps -ef | grep docker | grep {container_id_4}', timeout=300)
                lines = process_3.split('\n')
                # Look for the line containing "containerd-shim"
                pid_line = next(line for line in lines if 'containerd-shim' in line)
                # Extract the PID from the line
                pid_3 = pid_line.split()[1]
                print(f"Captured PID: {pid_3}")

                output = self.D4WAN.execute(f'run cd /proc/{pid_3}/fd && ls -ltr | wc -l', timeout=300)

                lines = output.split('\n')
                pid_3_before = lines[1].strip()



                print(f"Captured PID1 before: {pid_1_before}")
                print(f"Captured PID2 before: {pid_2_before}")
                print(f"Captured PID3 before: {pid_3_before}")
                ...
                # Print the container IDs
                for idx, match in enumerate(matches[1:], start=5):
                    variable_name = f'container_id_{idx}'
                    print(f'{variable_name} = {match}')

                    # Save the container IDs in separate variables
                    if idx == 5:
                        container_id_5 = match
                    elif idx == 6:
                        container_id_6 = match
                    elif idx == 7:
                        container_id_7 = match

                process_4 = self.D4WAN.execute(f'run ps -ef | grep docker | grep {container_id_5}', timeout=300)
                lines = process_4.split('\n')
                # Look for the line containing "containerd-shim"
                pid_line = next(line for line in lines if 'containerd-shim' in line)
                # Extract the PID from the line
                pid_4 = pid_line.split()[1]
                print(f"Captured PID: {pid_4}")

                output = self.D4WAN.execute(f'run cd /proc/{pid_4}/fd && ls -ltr | wc -l', timeout=300)
                lines = output.split('\n')
                pid_1_after = lines[1].strip()
                


                process_5 = self.D4WAN.execute(f'run ps -ef | grep docker | grep {container_id_6}', timeout=300)
                lines = process_5.split('\n')
                # Look for the line containing "containerd-shim"
                pid_line = next(line for line in lines if 'containerd-shim' in line)
                # Extract the PID from the line
                pid_5 = pid_line.split()[1]
                print(f"Captured PID: {pid_5}")


                output = self.D4WAN.execute(f'run cd /proc/{pid_5}/fd && ls -ltr | wc -l', timeout=300)
                lines = output.split('\n')
                pid_2_after = lines[1].strip()

                process_6 = self.D4WAN.execute(f'run ps -ef | grep docker | grep {container_id_7}', timeout=300)
                lines = process_6.split('\n')
                # Look for the line containing "containerd-shim"
                pid_line = next(line for line in lines if 'containerd-shim' in line)
                # Extract the PID from the line
                pid_6 = pid_line.split()[1]
                print(f"Captured PID: {pid_6}")

                output = self.D4WAN.execute(f'run cd /proc/{pid_6}/fd && ls -ltr | wc -l', timeout=300)
                lines = output.split('\n')
                pid_3_after = lines[1].strip()


                print(f"Captured PID1 before: {pid_1_before}")
                print(f"Captured PID2 before: {pid_2_before}")
                print(f"Captured PID3 before: {pid_3_before}")
                print(f"Captured PID1 after: {pid_1_after}")
                print(f"Captured PID2 after: {pid_2_after}")
                print(f"Captured PID2 after: {pid_3_after}")


                
                # Now, compare the numeric values
                if int(pid_1_after) <= int(pid_1_before) or int(pid_2_after) <= int(pid_2_before) or int(pid_3_after) <= int(pid_3_before):
                    log.info('Captured PID not increasing')
                else:
                    self.failed('Testcase failed: At least one captured PID increased.\n')
