...
def V4_Controller(failed, steps, script_args, testscript, testbed, test_data, timing):
    ...
    time.sleep(2)
    controller.execute('cd /root/MBB_MASTER/MIXED && ./rwa_mixed-clis_ucmp.sh', timeout = 300)
...
def Verifier_Before(failed, steps, script_args, testscript, testbed, test_data, timing):
    ...
    log.info(banner("Checking Labels"))
    counter = 0
    index = 0
    while 1:
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D18WAN.configure('interface FH0/7/0/0 shutdown\n'
                                    'interface FH0/13/0/10 shutdown\n'
                                    'interface FH0/13/0/24 shutdown\n'
                                    'interface FH0/7/0/1 shutdown\n'
                                    'interface FH0/7/0/1 shutdown\n'
                                    'interface FH0/13/0/32 shutdown\n'
                                    'interface FH0/13/0/33 shutdown\n'
                                    'interface Hu0/16/0/16 shutdown\n'
                                    'interface FH0/0/0/20 shutdown\n'
                                    'interface FH0/13/0/25 shutdown\n'
                                    'interface Hu0/16/0/30 shutdown\n'
                                    'interface FH0/7/0/16 shutdown\n'
                                    'interface FH0/13/0/35 shutdown\n'
                                    'interface FH0/16/0/20 shutdown\n'
                                    'interface FH0/16/0/21 shutdown', timeout=300)
                ...
                self.D18WAN.configure('no interface FH0/7/0/0 shutdown\n'
                                    'no interface FH0/13/0/10 shutdown\n'
                                    'no interface FH0/13/0/24 shutdown\n'
                                    'no interface FH0/7/0/1 shutdown\n'
                                    'no interface FH0/7/0/1 shutdown\n'
                                    'no interface FH0/13/0/32 shutdown\n'
                                    'no interface FH0/13/0/33 shutdown\n'
                                    'no interface Hu0/16/0/16 shutdown\n'
                                    'no interface FH0/0/0/20 shutdown\n'
                                    'no interface FH0/13/0/25 shutdown\n'
                                    'no interface Hu0/16/0/30 shutdown\n'
                                    'no interface FH0/7/0/16 shutdown\n'
                                    'no interface FH0/13/0/35 shutdown\n'
                                    'no interface FH0/16/0/20 shutdown\n'
                                    'no interface FH0/16/0/21 shutdown', timeout=300)
...
class TC2_CONTINUOUS_SWAN_PROGRAMMING(aetest.Testcase):
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for beloop in range(int(self.beloop)):
            ...
            time.sleep(1)
            self.controller.execute('cd /root/MBB_MASTER/MIXED && ./rwa_mixed-clis_ucmp.sh', timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                output = self.D18WAN.execute('show mpls forwarding prefix 100.100.110.1/32 location 0/7/CPU0', timeout=300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.D18WAN.execute('reload location 0/7/CPU0 noprompt', timeout = 7200)
                except:
                ...
                while True:
                    output = self.D18WAN.execute('sh platform | i 0/7/CPU0',timeout = 300)
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D18-4NH-1000CBF-6K-Impose-24x-UCMP.xml | curl -s -X POST http://172.25.124.54:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
                time.sleep(10)
                self.controller.disconnect()
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.D18WAN.configure('hw-module shutdown location 0/7/CPU0', timeout = 7200)
                except:
                ...
                try:
                    self.D18WAN.configure('no hw-module shutdown location 0/7/CPU0', timeout = 7200)
                except:
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.D8WAN.configure('interface FH0/0/0/11\n'
                                    'no bundle id 12800 mode active\n'
                                    'interface Hu0/4/0/11\n'
                                    'no bundle id 12801 mode active\n'
                                    'interface FH0/5/0/32\n'
                                    'no bundle id 12801 mode active\n'
                                    'interface FH0/5/0/33\n'
                                    'no bundle id 12801 mode active\n'
                                    'interface FH0/0/0/20\n'
                                    'no bundle id 12802 mode active\n'
                                    'interface FH0/4/0/15\n'
                                    'no bundle id 12803 mode active\n'
                                    'interface FH0/4/0/16\n'
                                    'no bundle id 12803 mode active\n', timeout=300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                self.controller.execute('gzip -c /root/MBB_MASTER/MIXED/ECMP/OWR1/OWR1_D8WAN_1000CBF_ECMP_nonshared_24x_HWPB.XML | curl -s     -X POST http://172.25.124.52:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gz    ip" --data-binary @-; echo;')
                time.sleep(10)
                self.controller.disconnect()
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                try:
                    self.D8WAN.configure('hw-module shutdown location 0/5/CPU0', timeout = 7200)
                except:
                    log.info("Lincard is shutting down")
                    time.sleep(300)
                ...
                try:
                    self.D8WAN.configure('no hw-module shutdown location 0/5/CPU0', timeout = 7200)
                except:
                    log.info("Lincard is booting")
                    time.sleep(300)
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                self.controller.execute('gzip -c /root/MBB_MASTER/MIXED/ECMP/OWR1/OWR1_D8WAN_1000CBF_ECMP_nonshared_24x_HWPB.XML | curl -s -X POST http://172.25.124.52:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
                time.sleep(10)
                self.controller.disconnect()
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            for beloop in range(int(self.beloop)):
                ...
                output = self.D18WAN.execute('run pidof agentxr', timeout = 300)
                output = output.split()

                PID1_before = output[-1]
                print(f"PID of Swanagent before: {PID1_before}")

                output = self.D18WAN.execute('run pidof vrf_relay', timeout = 300)
                ...
                output = self.D18WAN.execute(f'run cd /proc/{PID1_before}/fd && ls -ltr | wc -l', timeout=300)
                lines = output.split()
                tun_4_before = lines[-1]
                print(tun_1_before)
        

                output = self.D18WAN.execute(f'run cd /proc/{PID2_before}/fd && ls -ltr | wc -l', timeout=300)
                ...
                output = self.D18WAN.execute(f'run cd /proc/{PID3_before}/fd && ls -ltr | wc -l', timeout=300)
                ...
                output = self.D18WAN.execute(f'run cd /proc/{PID1_after}/fd && ls -ltr | wc -l', timeout=300)
                lines = output.split()
                tun_1_after = lines[-1]
                print(tun_1_after)
        

                output = self.D18WAN.execute(f'run cd /proc/{PID2_after}/fd && ls -ltr | wc -l', timeout=300)
                ...
                output = self.D18WAN.execute(f'run cd /proc/{PID3_after}/fd && ls -ltr | wc -l', timeout=300)
