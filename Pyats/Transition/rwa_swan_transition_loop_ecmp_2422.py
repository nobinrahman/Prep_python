...
def V4_Controller(failed, steps, script_args, testscript, testbed, test_data, timing):
    ...
    time.sleep(2)
    controller.execute('cd /root/MBB_MASTER/MIXED && ./rwa_mixed-clis_ecmp.sh', timeout = 300)
...
class TC2_CONTINUOUS_SWAN_PROGRAMMING(aetest.Testcase):
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for beloop in range(int(self.beloop)):
            ...
            time.sleep(1)
            self.controller.execute('cd /root/MBB_MASTER/MIXED && ./rwa_mixed-clis_ecmp.sh', timeout = 300)
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        #for loop in range(2):
            ...
            for beloop in range(int(self.beloop)):
                ...
                self.controller.execute('unset http_proxy')
                self.controller.execute('unset https_proxy')
                self.controller.execute('gzip -c /root/CBF_Only/734-40/OWR-734-50/D18-4NH-1000CBF-6K-Impose-24x-ECMP.xml | curl -s -X POST http://172.25.124.54:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;')
                time.sleep(10)
                self.controller.disconnect()
