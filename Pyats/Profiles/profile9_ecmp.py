...
class Profile9_ECMP(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            self.controller.execute('cd /root/MBB_MASTER/MIXED && ./profile9_ecmp.sh', timeout = 300)
            time.sleep(30)            
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            ...
            ###################### SHUT_INTERFACE ######################
            log.info(banner("Ingress_Default_All_2NHs"))
            module_args = {}
            module_args['timeout'] = 300
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            data_swan.append(["Ingress_Default_All_2NHs","Shut"] + get_values_non_zero(test_data))
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            data_swan.append(["Ingress_Default_All_2NHs","NoShut"] + get_values_non_zero(test_data))
            ...
            log.info(banner("Profile9_ECMP_SWAN_Convergence"))
            log.info(table_grid)

            file_path_swan = "/scratch/msftcvtauto/WAN/SWAN/CONVERGENCE/Logs/profile9_ecmp.csv"
