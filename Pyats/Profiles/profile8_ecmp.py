...
def New_Verifier_test(failed, steps, script_args, testscript, testbed, test_data, timing):
    ...
    while True:
        try:
            ...
            if outputDefault.count('path up') == 2 and outputScavenger.count('path up') == 3:
                time.sleep(60)
                break
        except Exception as e:
...
class Profile8_ECMP(aetest.Testcase):
    global coredump_list, showtech_list, interface_list
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for loop in range(int(self.loop)):
            ...
            self.controller.execute('cd /root/MBB_MASTER/MIXED && ./profile8_ecmp.sh', timeout = 300)
            time.sleep(30)            
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            ...
            ###################### SHUT_INTERFACE ######################
            log.info(banner("Ingress_Shared_All_2NHs"))
            module_args = {}
            module_args['timeout'] = 300
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            data_swan.append(["Ingress_Shared_All_2NHs","Shut"] + get_values_non_zero(test_data))
        ...
        if 'tgn' in test_data and 'spirent' == test_data['tgn']:
            data_swan.append(["Ingress_Shared_All_2NHs","NoShut"] + get_values_non_zero(test_data))
            ...
            log.info(banner("Profile8_ECMP_SWAN_Convergence"))
            log.info(table_grid)

            file_path_swan = "/scratch/msftcvtauto/WAN/SWAN/CONVERGENCE/Logs/profile8_ecmp.csv"
