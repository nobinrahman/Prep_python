...
def V4_Controller(failed, steps, script_args, testscript, testbed, test_data, timing):
    ...
    time.sleep(2)
    controller.execute('gzip -c /root/MBB_MASTER/MIXED/UCMP/OWR1/OWR1_D8WAN_SLICE_UCMP.XML | curl -s -X POST http://172.25.124.52:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
    time.sleep(60)
...
class TC2_CONTINUOUS_SWAN_PROGRAMMING(aetest.Testcase):
    ...
    def Node1_trigger(self, steps, script_args, testscript,testbed, test_data, timing):
        ...
        for beloop in range(int(self.beloop)):
            ...
            time.sleep(1)
            self.controller.execute('gzip -c /root/MBB_MASTER/MIXED/UCMP/OWR1/OWR1_D8WAN_SLICE_UCMP.XML | curl -s -X POST http://172.25.124.52:10000/flowtable -H "Content-Type:text/xml" -H "Content-Encoding: gzip" --data-binary @-; echo;', timeout = 300)
            time.sleep(1)
